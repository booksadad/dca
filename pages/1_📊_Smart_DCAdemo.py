    ...
    if not my_portfolio: 
        ...
        with st.expander("⏱️ [TIME MACHINE] สถิติโลกความจริงย้อนหลัง 3 ปี (หักค่าธรรมเนียมลากลแล้ว)", expanded=False):
            try:
                ...
                sum_w = sum(weights)
                weights = np.array(weights) / sum_w if sum_w > 0 else np.array([1/len(valid_tickers)] * len(valid_tickers))
                
                port_daily_ret = (daily_ret[valid_tickers] * weights).sum(axis=1) - (0.0020 / 252)
                port_cum = (1 + port_daily_ret).cumprod() * 100
                voo_cum = (1 + daily_ret['VOO']).cumprod() * 100
                
                fig_bt = px.line(pd.DataFrame({'พอร์ตของเรา': port_cum, 'S&P 500': voo_cum}), color_discrete_sequence=['#00d4ff', '#ff4b4b'])
                fig_bt.update_layout(height=350, margin=dict(l=10, r=10, t=30, b=10))
                st.plotly_chart(fig_bt, use_container_width=True)
                
                yp = len(daily_ret) / 252
