...
st.title("🛡️ HYBRID SENTINEL DCA (V.50.9 The Masterpiece)")
st.markdown("ระบบจัดพอร์ตระดับ 10/10 **(อัปเกรดสูงสุด: Semi-Variance + EWMA + Turnover Penalty!)**")
st.markdown("---")
...
engine_choice = st.sidebar.radio(
    ...
    [
        "🧠 Auto-Pilot (สมการ Utility + กลไก Advanced Quant)",
        "🛡️ Safe Mode (Risk Parity เน้นปลอดภัยสูงสุด)"
    ]
)

st.sidebar.markdown("---")
st.sidebar.subheader("🎛️ ปรับจูนความซิ่ง (Utility Parameter)")
lambda_risk = st.sidebar.slider(
    "ระดับการหลีกเลี่ยงความเสี่ยง (Risk Aversion: λ)", 
    min_value=0.1, max_value=10.0, value=2.0, step=0.1,
    help="λ ต่ำ = ซิ่งมิดไมล์ (เน้นผลตอบแทน), λ สูง = เพลย์เซฟ (เน้นความเสี่ยงต่ำ)"
)
st.sidebar.info("💡 **Active Modules:** \n- L2 Weight Penalty\n- L1 Turnover Penalty\n- EWMA Semi-Covariance (63 Days)")
st.sidebar.markdown("---")
...
sniper_msg = ""
actual_budget = budget # บล็อกงบไว้ที่ 500 ถ้วน!

if st.button("🚀 รันระบบ Ultimate Rebalancer"):
    if not my_portfolio: 
        st.error("⚠️ โปรดระบุชื่อหุ้นที่แถบด้านข้างซ้ายมือก่อนครับ")
    else:
        status_box = st.status(f"🔮 กำลังประมวลผลวิเคราะห์เชิงลึกด้วย Advanced Math...", expanded=True)
        ...
        # ==========================================
        # ⚖️ THE QUANT MASTERPIECE ENGINE (V.50.9)
        # ==========================================
        if "Auto-Pilot" in engine_choice:
            status_box.update(label=f"🧮 เดินเครื่องสมการ: EWMA Semi-Covariance + L1/L2 Penalties...")
            ...
            try:
                ...
                for idx, row in port_df.iterrows():
                    ...
                    if len(series) >= 20:
                        ...
                        if current_p <= lower_band or rsi_val <= 40: 
                            dynamic_boost = max(0, (40 - rsi_val) / 50) 
                            mu[t] = max(mu.get(t, 0), 0) + 0.15 + dynamic_boost 
                            sniper_targets.append(t)

                if len(sniper_targets) > 0:
                    sniper_msg = f"🎯 **Sniper Alert:** เรดาร์พบเป้าหมายหลุดกรอบราคา ได้แก่ **{', '.join(sniper_targets)}** ระบบได้เพิ่มน้ำหนักการซื้อให้แล้ว!"

                # 🌟 Advanced Module 1: Semi-Variance (ฟิลเตอร์เอาเฉพาะขาลงมาคิดความเสี่ยง)
                downside_port_returns = port_returns.copy()
                downside_port_returns[downside_port_returns > 0] = 0
                
                # 🌟 Advanced Module 2: EWMA (ให้น้ำหนักข้อมูล 3 เดือนล่าสุดหนักที่สุด)
                last_date = downside_port_returns.index[-1]
                ewma_cov = downside_port_returns.ewm(span=63).cov().loc[last_date] * 252
                
                num_assets = len(port_tickers)
                
                # ดึงสัดส่วนพอร์ตปัจจุบันมาใช้กับ Turnover Penalty
                current_weights_arr = (port_df['Current'] / total_port_value).fillna(0).values if total_port_value > 0 else np.zeros(num_assets)
                
                # ตัวแปรควบคุมความงกและความโลภ
                gamma_penalty = 0.5   # L2: ป้องกัน All-in 100% ตัวเดียว
                tau_penalty = 0.02    # 🌟 Advanced Module 3: L1 Turnover Penalty (คุมค่าธรรมเนียม)

                def neg_utility(w):
                    expected_return = np.sum(mu * w)
                    # ใช้ EWMA Semi-Covariance แทนแบบเก่า
                    ...
                    utility = expected_return - (lambda_risk / 2.0) * portfolio_variance - l2_penalty - l1_turnover_penalty
                    return -utility
                ...
                st.warning(f"⚠️ ตลาดผันผวนจนสมการหาจุดลงตัวไม่ได้ ระบบสลับไปใช้ 'Risk Parity' ชั่วคราวครับ! ({e})")
        ...
        st.markdown(f"### 📋 3. ตารางใบสั่งซื้ออัจฉริยะ (Advanced Utility Engine)")
