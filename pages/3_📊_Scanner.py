...
if st.button("🚀 เดินเครื่องเรดาร์สแกนตลาด", type="primary"):
    with st.spinner("⏳ กำลังกวาดข้อมูลจาก Wall Street และรันสมการ Z-Score... (อาจใช้เวลา 10-20 วินาที)"):
        ...
        # 🛠️ แก้บั๊กกราฟ Plotly: ปรับฐานคะแนน Alpha ให้เป็นบวกทั้งหมดเพื่อใช้กำหนดขนาด Bubble
        df_alpha['Bubble_Size'] = df_alpha['Total_Alpha_Score'] - df_alpha['Total_Alpha_Score'].min() + 1
        
        # วาดกราฟ Scatter Plot ให้เห็นภาพรวม
        fig = px.scatter(
            df_alpha, x="Z_Quality", y="Z_Mom", text="Ticker", 
            size="Bubble_Size", # ใช้คอลัมน์ใหม่ที่ไม่มีค่าติดลบแล้ว
        ...
        # ขีดเส้นแกน 0 แบ่งโซน 4 ควอดรันต์
        fig.add_hline(y=0, line_dash="dot", line_color="gray", opacity=0.5)
        fig.add_vline(x=0, line_dash="dot", line_color="gray", opacity=0.5)
        
        st.plotly_chart(fig, use_container_width=True)
        # 💾 สร้างสะพานเชื่อม: บันทึก Top 5 ส่งไปให้บอร์ดบริหาร
        top_5_tickers = display_df['หุ้น'].head(5).tolist()
        pd.DataFrame({'Alpha_Tickers': top_5_tickers}).to_csv("alpha_radar.csv", index=False)
        st.info(f"💾 ส่งแฟ้มรายชื่อจ่าฝูง ({', '.join(top_5_tickers)}) เข้าสู่ห้องประชุมบอร์ดบริหารเรียบร้อยแล้ว!")
